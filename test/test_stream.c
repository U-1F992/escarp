...
int test_callback_2(escarp_stream_t *stream, void *other_data) {
    assert(stream != NULL);
    assert(0 == escarp_getc(stream));
    escarp_with_backtracking(stream, test_callback_3, other_data);
    assert(1 == escarp_getc(stream));
    escarp_with_backtracking(stream, test_callback_4, other_data);
    assert(4 == escarp_getc(stream));
    return 1;
}
...
int main(void) {
    ...
    assert(0 ==
           escarp_with_backtracking(&(stream.base), test_callback_0, NULL));
    assert(0 == escarp_getc(&(stream.base)));
    assert(1 == escarp_getc(&(stream.base)));
    assert(2 == escarp_getc(&(stream.base)));
    assert(0 !=
           escarp_with_backtracking(&(stream.base), test_callback_1, NULL));
    assert(5 == escarp_getc(&(stream.base)));
    assert(EOF == escarp_getc(&(stream.base)));

    uint8_array_stream_init(&stream, (unsigned char[]){0, 1, 2, 3, 4, 5}, 6);
    escarp_with_backtracking(&(stream.base), test_callback_2, NULL);
    escarp_with_backtracking(&(stream.base), test_callback_5, NULL);
    assert(5 == escarp_getc(&(stream.base)));
    assert(EOF == escarp_getc(&(stream.base)));
